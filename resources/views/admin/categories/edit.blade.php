...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST"
            ...
            <x-input name="name" label="Nombre" :value="old('name', $category->name)" required />
            <x-input name="slug" label="Slug" :value="old('slug', $category->slug)" />

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                <textarea name="description" rows="4"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 bg-gray-50 hover:bg-white focus:bg-white transition-all">{{ old('description', $category->description) }}</textarea>
            </div>
